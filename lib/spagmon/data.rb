...
module Spagmon
  class Data
    ...
    class << self
      ...
      # Try to guess the Spagmon home directory, by first looking at the
      # `SPAGMON_HOME` environment variable, then '/etc/spagmon', then '~/.spagmon'
      # @return [String]
      def detect_home_dir
      ...
      # @return [String|NilClass]
      def root_home_dir
        (Spagmon.root? || Dir.exist?(ROOT_HOME)) && ROOT_HOME
      end
    ...
    # Returns true if using root data at /etc/spagmon, or false if using user data
    # is at ~/.spagmon
